...
export class Spells {
    ...
    private getNextOpponent(position: number, isLeft = true): Player | null {
        ...
        let targetPosition;
        if (isLeft) {
            targetPosition = currentPlayerPosition === alivePlayers.length - 1 ? 0 : currentPlayerPosition + 1;
        } else {
            targetPosition = currentPlayerPosition === 0 ? alivePlayers.length - 1 : currentPlayerPosition - 1;
        }
    ...
    private getLeftOpponent(position: number): Player | null {
        return this.getNextOpponent(position);
    }

    private getRightOpponent(position: number): Player | null {
        return this.getNextOpponent(position, false);
    }
    ...
    private useChronoWalker = async (positionPlayer: number): Promise<void> => {
        ...
        rollResults.push({
            ...
            rollResult: sumArray(playerResult)
        });
        // бросок соперников
        const opponentResults = await makeDiceRolls(opponents);
        rollResults.push(...opponentResults.map(({
            player,
            rolls
        }) => ({
            player,
            rollResult: sumArray(rolls)
        })));

        if (rollResults.length < 1) return;
        rollResults.sort((a, b) => a.rollResult - b.rollResult);
        let target: Player;
        if (rollResults[0].rollResult !== rollResults[1].rollResult) {
            target = rollResults[0].player;
        } else {
            target = await throwCheck([rollResults[0].player, rollResults[1].player], false);
        }
    ...
    private useCatTrouble = async (positionPlayer: number): Promise<void> => {
        ...
        diceResults.push({
            player: currentPlayer,
            roll: sumArray(currentPlayerRoll)
        });
        ...
        rolls.forEach((rollResult) => {
            diceResults.push({
                ...
                roll: sumArray(rollResult.rolls)
            });
        });
    ...
    private useCubic = async (positionPlayer: number): Promise<void> => {
        ...
        const targets: Array<{ target: Player, damage: number }> = opponentsResults
            ...
            .map(({
                ...
                {
                    target: player,
                    damage: resultDiceRollPlayer[resultDiceRollPlayer.indexOf(rolls[0])] || 0
                }
            ));

        if (targets.length === 0) return;
        await Promise.race(targets.map(({
            target,
            damage
