...
function race<T>(task: Promise<T>, data?: T, time?: number): Promise<T> {
    return new Promise<T>((resolve) => {
        delay(time ? time + SERVER_DELAY : MAX_AWAIT_TIME + SERVER_DELAY)
            .then(() => {
                resolve(data as T);
            });
        task.then(resolve)
...
export class Player {
    ...
    get handCards(): Array<ICard> {
        return this.handCardsValue;
    }

    get isSpellReady(): boolean {
        return this.spell !== PlayerSpell.Empty;
    }

    get spell(): PlayerSpell {
    ...
    private addSpellCards(cardIds: Array<string>): void {
        ...
        const message: ISpellSelected = {
            playerId: this.id,
            spellCards: this.currentSpell.length
        };
