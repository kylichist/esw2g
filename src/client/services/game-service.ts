...
export class GameService {
    ...
    get currentPlayerCards(): Array<ICard> {
        return this.playerCards;
    }

    get currentPlayers(): Array<IPlayerInfo> {
        return this.players;
    }

    get isCasting(): boolean {
        return this.isCastingStep;
    }
    ...
    async joinGame(gameId: string): Promise<void> {
        ...
        if (joinResponse.isStarted && this.onGameStarted) {
            this.onGameStarted(false, joinResponse.timeout);
    ...
    private async healthUpdate(message: IHealthUpdate): Promise<IHubResponse<null | string>> {
        ...
        if (player) {
            (<IPlayerInfo>player).health = message.currentHealth;
        } else {
            return HubResponse.Error('Player not found');
        }
